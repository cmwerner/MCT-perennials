...
Both of these also add in average expectations. *At the moment these are the mean of the wet and the dry conditions, but we want to change that later to be weighted by the ratio of wet to dry conditions -- which I think will be easiest if we move that part to a later step so it's part of running the function rather than the setup.* 
```{r parameters}
...
*this changes with the species codes in the call to the partition_epsilons() function*
```{r partition-run}
