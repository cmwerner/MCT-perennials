...
if(pair == 'as.an'){
  load('mct-partitions-as_2.RData')
  species_names <- c(
    an = 'AN invades AS',
    as = 'AS invades AN'
  )
  file.out <- 'partitions-AS-AN_env-ratio.pdf'
}
if(pair == 'p.an'){
  load('mct-partitions-p.RData')
  species_names <- c(
    an = 'AN invades P',
    s.p = 'P invades AN'
  )
  file.out <- 'partitions-P-AN_env-ratio.pdf'
}
...
part.sum <- part.long %>% 
  ...
  dplyr::summarise(
    ...
    delta.se = sd(delta)/sqrt(length(delta))
  )
...
ggplot(part.sum, aes(x = env.ratio, y = delta.mean, 
                     ymin = delta.mean - delta.se, 
                     ymax = delta.mean + delta.se, 
                     color = partition, fill = partition)) +
  facet_grid(cols = vars(sp.invader),
             labeller = labeller(sp.invader = species_names)) + 
  geom_line() +
 # geom_ribbon() +
