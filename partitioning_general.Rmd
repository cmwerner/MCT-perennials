...
pars.as.an <- read.csv('model_parameters_annuals_lambda_opposite.csv') %>%
  mutate(stem.surv = NULL) 

# perennial - nonseedbanking annual parameters
pars.p.an <- read.csv('model_parameters_perennials_lambda_opposite.csv') %>%
...
if(pair == 'as.an'){
  # load('mct-partitions_as-lambda-same.RData')
  load('mct-partitions_as-lambda-opposite.RData')
  species_names <- c(
    an = 'AN invades AS',
    as = 'AS invades AN'
  )
   #file.out <- 'partitions_as-lambda-same_env-ratio.pdf'
   file.out <- 'partitions_as-lambda-opposite_env-ratio.pdf'
}
if(pair == 'p.an'){
  # load('mct-partitions_p-lambda-same.RData')
  load('mct-partitions_p-lambda-opposite.RData')
  species_names <- c(
    an = 'AN invades P',
    s.p = 'P invades AN'
  )
  # file.out <- 'partitions_p-lambda-same__env-ratio.pdf'
   file.out <- 'partitions_p-lambda-opposite_env-ratio.pdf'
}
...
ggplot(filter(part.sum, env.ratio == 0.5), 
  ...
  facet_grid(cols = vars(sp.invader),
             labeller = labeller(sp.invader = species_names)) + 
  geom_errorbar(position = position_dodge(), color = 'grey50') +
  geom_bar(stat = 'identity', position = position_dodge()) +
  theme_cw() +
  ylab('Partitioning of growth rate when rare') +
  scale_x_discrete(labels = c('full' = 'Full',  'e0' = expression(Delta^0),
