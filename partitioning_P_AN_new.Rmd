...
```{r running-invasion}
run_invasion <- function(sp.invader, sp.resident, N0.r, parameters, 
  ...
  # populations with resident species impacted by the invader
  n.res.impact <- tibble(an = as.numeric(rep(NA, total.time.steps)),
  ...
  n.res.impact[1,sp.resident] <- N0.r
  n.res.impact[1,sp.invader] <- 0
  ...
  for(t in 1:init.time.steps){
    ...
    #NOTE: still need to do this for alphas--mutate with an across() function?
    
    # running one time step with the appropriate environmental parameters
    n.all[t+1,] <- pop_interactions(N0 = n.all[t,], param = p.env)
    n.res.impact[t+1,] <- n.all[t+1,]
    
  }
  
  # running the resident and invader for the remaining time steps
  # NOTE: not worrying about resetting the resident for each time step here yet, 
  # just allowing it to have an impact to get a framework in
  for (t in (init.time.steps+1):(total.time.steps-1)){
    ...
    N0.set <- n.all[t,]
    N0.set[sp.invader] <- 1/length(sp.invader) # NOTE: change this later to reflect equilibrium proportion?
    # running one time step with the appropriate environmental parameters
    n.all[t+1,] <- pop_interactions(N0 = N0.set, param = p.env)
...
test.run <- run_invasion(sp.invader = c('s','p'), sp.resident = 'an', N0.r = 100, 
                         parameters = pars, 
                         vary.lambda = TRUE, vary.alpha = TRUE, 
                         init.time.steps = 50, total.time.steps = 100)
...
*Need to modify output and/or plotting and partitioning functions so they match up*
