...
run_invasion <- function(sp.invader, sp.resident, N0.r, parameters, 
  ...
  # name of column(s) for resident species growth rate
  sp.res.gr <- paste(sp.resident, 'gr', sep = '.')
  # name of column(s) for invader species growth rate
  sp.inv.gr <- paste(sp.invader, 'gr', sep = '.')
  ...
  for(t in 1:init.time.steps){
    ...
    if(vary.alpha == FALSE){
      p.env[,sp.resident] <- p.ave[,sp.resident]
      p.env[,sp.invader] <- p.ave[,sp.invader]
    }
  ...
  for (t in (init.time.steps+1):(total.time.steps-1)){
    ...
    N0.together <- n.all[t,]
    N0.invader <- 1/length(sp.invader) # NOTE: change later to equilibrium proportion?
    N0.together[sp.invader] <- N0.invader
    # running one time step with the appropriate environmental parameters
    ...
    n.all[t+1,sp.res.gr] <- n.all[t+1,sp.res.impact]/n.all[t,sp.resident]
    
    # growth rate of the invader
    n.all[t+1,sp.inv.gr] <- n.all[t+1,sp.invader]/N0.invader
