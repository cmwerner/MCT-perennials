...
run_invasion <- function(sp.invader, sp.resident, N0.r, parameters, 
  ...
  # name of column for resident species pop when impacted by invading pop
  ...
  for (t in (init.time.steps+1):(total.time.steps-1)){
    ...
    N0.together <- n.all[t,]
    N0.together[sp.invader] <- 1/length(sp.invader) # NOTE: change later to equilibrium proportion?
    # running one time step with the appropriate environmental parameters
...
*NEXT STEP is to get the partition function working (I think it should be nearly compatible with this setup)*
