...
*TO DO: Still need to look at storage effect sub-partition. Also want to adjust to return both invader and resident terms with a separate function/wrapper to calculate the deltas, so we have the option of looking at these individually. Also need to do code checks for long-term coexistence using these parameters in a more systematic way*
...
partition_epsilons <- function(sp.inv, sp.res, pars, 
  ...
  # TO DO: NEED TO CHECK CORRELATION/COVARIANCE COMPONENT
  # no correlation
...
test.1 <- partition_epsilons(sp.inv = sp.2, sp.res = sp.1, pars, 
                             env.condition, time.warm.up, time.full)
test.2 <- partition_epsilons(sp.inv = sp.1, sp.res = sp.2, pars, 
                             env.condition, time.warm.up, time.full)
...
Verifying that parameters can result in long-term coexistence in conditions predicted by positive low-density growth rates
...
coexistence_run <- function(sp.invader, sp.resident, env.ratio, 
  ...
  # running just sp 1 at the beginning
  for(t in 1:time.warm.up){
     # parameters for the varying env condition
  ...
  n.all[time.warm.up,sp.invader] <- N0.i
  
  for(t in time.warm.up:time.full){
    # parameters for the varying env condition
    p.env <- pars %>% filter(treatment == env.condition[t])
...
for(env.ratio in env.seq){
  # run with sp.1 initialized first
  n.ave.1 <- coexistence_run(sp.invader = sp.1, sp.resident = sp.2, 
                             env.ratio, time.warm.up, time.full, 
                             N0.i = N0.1, N0.r = N0.2)
  coexistence.env <- rbind(coexistence.env, n.ave.1)
  # repeat with sp.2 initialized first
  n.ave.2 <- coexistence_run(sp.invader = sp.2, sp.resident = sp.1, 
                             env.ratio, time.warm.up, time.full, 
                             N0.i = N0.2, N0.r = N0.1)
  coexistence.env <- rbind(coexistence.env, n.ave.2)
}


print(coexistence.env)

coexist.env.long <- coexistence.env %>% pivot_longer(cols = c(sp.1, sp.2), names_to = "sp", values_to = "population")

ggplot(coexist.env.long, aes(x = env.ratio, y = population, color = sp)) +
  facet_grid(cols = vars(sp.invader),
             labeller = labeller(sp.invader = species_names)) + 
  geom_line() +
  theme_cw() +
