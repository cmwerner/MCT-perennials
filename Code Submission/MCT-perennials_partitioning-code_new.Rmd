...
*Re-working for revisions, more closely matching Ellner et al 2019 approach*
...
*Edits -- changing this to do all the different partitions at each time step. Warm-up time is unchanged, only doing the partitions in the main time. Going to start by just doing the full run with the changed data frame set up and make sure that still works*

All the column names are based on the actual species names

This function will return a dataframe with columns: time, env.cond, r.full.no.i, r.full.with.i, i.full, r.none, i.none, r.no.lambda, i.no.lambda, r.no.alpha, i.no.alpha
The next section of the code can use this to calculate growth rates and average across the time series. 
*except maybe not quite because right now we're using separate an/as/s/p columns? so should we do that too? is it useful to keep s and p separate if we're just combining them back together later?*
...
run_invasion <- function(sp.invader, sp.resident, N0.r, parameters, env.cond,
  ...
  # tibble for output
  partition.runs.output <- tibble(time = 1:total.time.steps,
                                  env.cond, 
                                  r.full.no.i = (rep(NA, total.time.steps)),  # this will be the only one with data for the warm-up period
                                  r.full.with.i = (rep(NA, total.time.steps)),
                                  i.full = (rep(NA, total.time.steps)),
                                  r.none = (rep(NA, total.time.steps)),
                                  i.none = (rep(NA, total.time.steps)),
                                  r.no.lambda = (rep(NA, total.time.steps)),
                                  i.no.lambda = (rep(NA, total.time.steps)),
                                  r.no.alpha = (rep(NA, total.time.steps)),
                                  i.no.alpha = (rep(NA, total.time.steps)))
  ...
  # first row in tibble output
  partition.runs.output[1,'r.full.no.i'] <- sum(N0.r)
  ...
  # CHECK: Warm up only for full run, because partitions happen at each of the post-warm-up time steps?
  
  for(t in 1:init.time.steps){
    # parameters for the varying env condition -- this is the only one we use for the warm-up
    ...
    # save resident population size in output tibble
    # sum in case it's a multi-stage population for full population size
    partition.runs.output[t+1,'r.full.no.i'] <- sum(n.all[t+1,sp.resident])
  ...
  # THIS IS ALL GOING TO CHANGE/SHUFFLE: ---------- 
