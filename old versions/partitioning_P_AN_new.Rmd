...
```{r parameters}
# main parameters
pars <- read.csv('model_parameters_perennials_theory.csv') %>%
...
run_invasion <- function(sp.invader, sp.resident, N0.r, parameters, env.cond,
  ...
  for(t in 1:init.time.steps){
    ...
    lambda.columns <- c('lambda','seed.germ','stem.surv') # NOTE would need to change for a generalized function with different parameter columns
...
invade.an.full <-  run_invasion(sp.invader = 'an', sp.resident = c('s','p'), N0.r = 100, 
                         parameters = pars, env.cond = env.condition,
                         vary.lambda = TRUE, vary.alpha = TRUE, 
                         covary = TRUE,
                         init.time.steps = time.warm.up, total.time.steps = time.full)
...
partitions <- partition_epsilons(sp.inv = c('s','p'), sp.res = 'an') %>%
  rbind(partition_epsilons(sp.inv = 'an', sp.res = c('s','p'))) %>%
...
ggplot(partitions, aes(x = partition, y = delta, fill = partition)) + 
 ...
 #ggsave('partitions_P_AN.pdf', width = 8, height = 5, units = 'in')
