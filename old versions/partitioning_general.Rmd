...
plot_invasion <- function(df){ # plots actual population sizes
  df.long <- df %>% 
    pivot_longer(cols = !c(time, env, inv, res, run, starts_with('gr')),
...
invade.1.full <-  run_invasion(sp.invader = sp.1, sp.resident = sp.2, N0.r = 100, 
...
invade.2.full <-  run_invasion(sp.invader = sp.2, sp.resident = sp.1,
...
# ggsave('invasion-example-1.pdf', width = 5, height = 3, units = 'in')
plot_gr(invade.1.full)
# ggsave('invasion-example-2.pdf', width = 5, height = 3, units = 'in')


plot_invasion(filter(invade.2.full, time > time.warm.up))
# ggsave('invasion-example-3.pdf', width = 5, height = 3, units = 'in')
plot_gr(invade.2.full)
# ggsave('invasion-example-4.pdf', width = 5, height = 3, units = 'in')


invade.1.long <- invade.1.full %>%
...
test1 <- invade.1.long %>% 
    filter(species == sp.1.name) %>%
    select(gr.log) %>% colMeans()

test2 <- invade.1.long %>% 
    filter(species == sp.2.name) %>%
    select(gr.log) %>% colMeans()
...
* DON'T RUN THIS * Same as above but doing multiple runs for troubleshooting
...
test <- invade.runs %>% filter(run == 1, inv == sp.1.name)
...
sp.res.gr <- invade.runs %>% group_by(inv, res, run) %>%
  dplyr::summarise(
    gr.sp.mean = mean(log(gr.s.p)),
    gr.an.mean = mean(log(gr.an))
  ) %>% filter(res == 's.p')

which(abs(sp.res.gr$gr.sp.mean) > 0.005)

sp.inv.gr <- invade.runs %>% group_by(inv, res, run) %>%
  dplyr::summarise(
    gr.sp.mean = mean(log(gr.s.p)),
    gr.an.mean = mean(log(gr.an))
  ) %>% filter(inv == 's.p')
which(abs(sp.inv.gr$gr.sp.mean) > 0.2)

hist(sp.inv.gr$gr.sp.mean)

test.1 <- filter(invade.runs, run == 1, inv == 's.p')
test.2 <- filter(invade.runs, run == 2, inv == 's.p')
plot_gr(test.1)
ggsave(filename = 'test1_gr.pdf')
plot_gr(test.2)
ggsave(filename = 'test2_gr.pdf')

plot_invasion(test.1)
ggsave(filename = 'test1_pop.pdf')
plot_invasion(test.2)
ggsave(filename = 'test2_pop.pdf')
...
plot.out.env.inv <- here(paste0('figures/', file.core, '_env-ratio_inv.pdf')) 
plot.out.env.res <- here(paste0('figures/', file.core, '_env-ratio_res.pdf')) 
...
#cb_palette <- c("black", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00")
#cb_palette <- c("black", "#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00")
cb_palette <- c("black", "#009E73", "#56B4E9", "#E69F00","#D55E00","#0072B2")


# uncombining invader vs resident subfigs for now
ggplot(filter(part.sum, sp.invader == "s.p"), aes(x = env.ratio)) +
  ...
  ggsave(plot.out.env.inv, width = 5, height = 5, units = 'in')
  
  ggplot(filter(part.sum, sp.invader == 'an'), aes(x = env.ratio)) +
  geom_ribbon(alpha = 0.3, aes(ymin = delta.mean - delta.sd, 
                               ymax = delta.mean + delta.sd,
                               fill = partition)) +
  geom_line(aes(y = delta.mean, color = partition)) +
  ...
  ggsave(plot.out.env.res, width = 5, height = 5, units = 'in')
...
ggplot(filter(part.long, env.ratio == 0.5, partition == 'full', sp.invader == 's.p'), 
       aes(inv)) +
  geom_histogram()
...
## New plotting functions to generate our final figures
Using the data from the file run on the computer cluster
```{r partition-plots-Figure-2}

# nested for loops is fine, function would probably be a little cleaner
part.sum.all <- tibble()
...
# graph by proportion of env intrinsically favorable to our focal species
# (perennial or seedbanking annual) -- in the original parameter lists
...
# 
# # Figure 2 barchart showing strength of partitions at 50-50 env
# # with the perennial as the invader
# part.sum.50.p <- part.sum.all %>% filter(sp.invader == 's.p', env.ratio == 0.5,
#                                        env.case == 'same.flipped')
# part.sum.50.as <- part.sum.all %>% filter(sp.invader == 'as', env.ratio == 0.5,
#                                        env.case == 'same.flipped')
# 
# plot.50.p <- part.sum.50.p %>% 
#   ggplot(aes(x = partition, fill = partition, y = delta.mean,
#                            ymin = delta.mean - delta.sd, ymax = delta.mean + delta.sd)) +
#   geom_col() +
#   geom_errorbar(width = 0.2, col= 'grey30') +
#   annotate("segment", x = 1.5, xend = 1.5, y = -0.17, yend = 0.42, colour = 'grey50') +
#   annotate("segment", x = 5.5, xend = 5.5, y = -0.17, yend = 0.42, colour = 'grey50') +
#   annotate("rect", xmin = 5.5, xmax = 7.65, ymin = -0.17, ymax = 0.42,
#   alpha = .2) +
#   coord_cartesian(ylim = c(-0.05, 0.4)) +
#   scale_fill_manual(values = cb_palette_long, labels = partition.legend, name = 'Partition') +
#   scale_x_discrete(labels = partition.legend) +
#   theme_cw() +
#     guides(fill = "none") +
...
#   xlab('Partition') +
#   ylab('Partitioning of low density growth rate') +
#   ggtitle('Invader: Perennial \n Resident: Nonseedbanking Annual')
# 
# plot.50.as <- part.sum.50.as %>% 
#   ggplot(aes(x = partition, fill = partition, y = delta.mean,
#                            ymin = delta.mean - delta.sd, ymax = delta.mean + delta.sd)) +
#   geom_col() +
#   geom_errorbar(width = 0.2, col= 'grey30') +
#   annotate("segment", x = 1.5, xend = 1.5, y = -0.18, yend = 0.8, colour = 'grey50') +
#   annotate("segment", x = 5.5, xend = 5.5, y = -0.18, yend = 0.8, colour = 'grey50') +
#   annotate("rect", xmin = 5.5, xmax = 7.65, ymin = -0.18, ymax = 0.8,
#   alpha = .2) +
...
#   xlab('Partition') +
#   ylab('') +
#   ggtitle('Invader: Seedbanking Annual \n Resident: Nonseedbanking Annual')
# 
# plot.50.p + plot.50.as

# ggsave('mct-partitions_figure-2.pdf', width = 8, height = 5, units = 'in')
...
# With the additional partitions full, interaction effect, and additional partition
