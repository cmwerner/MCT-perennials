...
*Don't re-run this, just skip to the next step to load it* Full variation with multiple runs for troubleshooting (no partitioning). This saves at the end and loads at the next step so we don't have to rerun it for time.
...
sp.res <- invade.runs %>% filter(res == 'an')
sp.res <- droplevels(sp.res)
hist(sp.res$gr.an) # not tri-modal if we just take the means
hist(log(sp.res$gr.an))


sp.res.gr2 <- sp.res %>% group_by(run) %>%
    dplyr::summarise(
    gr.an.log.mean = mean(log(gr.an)),
    gr.an.mean = mean(gr.an)
  )
...
hist(test.1$gr.an, ylim = c(0, 250), breaks = seq(0,8, by=0.125)) 
...
mean(log(test.1$gr.an), na.rm = TRUE) # high
mean(log(test.3$gr.an), na.rm = TRUE) # near 0
mean(log(test.11$gr.an), na.rm = TRUE) # low
...
Is this because of within run structure? What if we permute run labels?
...
sp.res <- invade.runs %>% filter(res == 'an')
sp.res <- droplevels(sp.res)
fakeruns <- sample(20, 6980, replace=TRUE)
sp.res <- cbind(sp.res, fakeruns)

hist(sp.res$gr.an) # not tri-modal if we just take the means
hist(log(sp.res$gr.an))

sp.res.gr2 <- sp.res %>% group_by(fakeruns) %>%
    dplyr::summarise(
    gr.an.log.mean = mean(log(gr.an)),
    gr.an.mean = mean(gr.an)
  )
...
hist(sp.res.gr2$gr.an.mean) # not tri-modal if we just take the means
hist(sp.res.gr$gr.an.log.mean) # NOT tri-modal if we permute the labels
which(sp.res.gr$gr.an.log.mean > 0.004)
which(sp.res.gr$gr.an.log.mean < -0.004)

xx <- acf(test.1$gr.an) # strong lag-1 negative correlation
acf(test.11$gr.an)

# if we break this, I'll bet we don't get the autocorrelation
gr.ex <- sample(sp.res$gr.an, 6980, replace=FALSE)
fake.data <- data.frame(gr.ex, sp.res$run, sp.res$time)
colnames(fake.data) <- c("gr.an", "run", "time")

fake.test.1 <- filter(fake.data, run == 1)
fake.test.3 <- filter(fake.data, run == 3)
fake.test.11 <- filter(fake.data, run == 11)

fake.xx <- acf(fake.test.1$gr.an) # strong lag-1 negative correlation
acf(fake.test.11$gr.an)

sp.res.gr3 <- fake.data  %>% group_by(run) %>%
    dplyr::summarise(
    gr.an.log.mean = mean(log(gr.an)),
    gr.an.mean = mean(gr.an)
  )

hist(sp.res.gr3$gr.an.mean) # not tri-modal if we just take the means
hist(sp.res.gr3$gr.an.log.mean)

```

Ok, so it seems to be something about model and the timeseries structure. I want to dive into that a little bit more...

```{r environmental dive}
env.series <- rep(NA, dim(sp.res)[1])
counter <- 1

for (xx in 1:20) {
  tempdata <- filter(sp.res, run == xx)
  counter <- counter + 1 # counting for the first timepoint per run
  for (yy in 2:dim(tempdata)[1]) {
    env.series [counter] <- paste(tempdata$env[yy-1], 
                                  tempdata$env[yy], sep ="_")
    counter <- counter + 1
  }
}

sp.res.env <- cbind(sp.res, env.series)

# there are all about the same, which is correct and makes sense
# so why is there this neg. ar(1) autocorrelation?
length(which(sp.res.env$env.series == "dry_wet"))
length(which(sp.res.env$env.series == "dry_dry"))
length(which(sp.res.env$env.series == "wet_wet"))
length(which(sp.res.env$env.series == "wet_dry"))

dry_wet <- filter(sp.res.env, env.series == "dry_wet")
hist(dry_wet$gr.an) # hmm, that's bimodal

wet_wet <- filter(sp.res.env, env.series == "wet_wet")
hist(wet_wet$gr.an) # hmm, that's trimodal?...

wet_dry <- filter(sp.res.env, env.series == "wet_dry")
hist(wet_dry$gr.an) # hmm, that's also trimodal. Looks almost identical to above

dry_dry <- filter(sp.res.env, env.series == "dry_dry")
hist(dry_dry$gr.an) # hmm, that's bimodal. Looks a ton like dry_wet

# Does it depend on the last 3 conditions?...
env.series.long <- rep(NA, dim(sp.res)[1])
counter <- 1

for (xx in 1:20) {
  tempdata <- filter(sp.res, run == xx)
  counter <- counter + 2 # counting for the first timepoint per run
  for (yy in 3:dim(tempdata)[1]) {
    env.series.long [counter] <- paste(tempdata$env[yy-2],
                                  tempdata$env[yy-1], 
                                  tempdata$env[yy], sep ="_")
    counter <- counter + 1
  }
...
sp.res.env.long <- cbind(sp.res.env, env.series.long)

# these should all be about 870ish long
# looks good
ggplot(sp.res.env.long) +
  geom_bar(aes(as.factor(env.series.long)))

wet_dry_dry <- filter(sp.res.env.long, env.series.long == "wet_dry_dry")
hist(wet_dry_dry$gr.an) # bimodal, but all between .12 and .16

wet_dry_wet <- filter(sp.res.env.long, env.series.long == "wet_dry_wet")
hist(wet_dry_wet$gr.an) # bimodal, but all between .12 and .16

wet_wet_wet <- filter(sp.res.env.long, env.series.long == "wet_wet_wet")
hist(wet_wet_wet$gr.an) # all close to 1 (range 1-1.28)

wet_wet_dry <- filter(sp.res.env.long, env.series.long == "wet_wet_dry")
hist(wet_wet_dry$gr.an) # all close to 1 (range 1-1.28)

dry_dry_dry <- filter(sp.res.env.long, env.series.long == "dry_dry_dry")
hist(dry_dry_dry$gr.an) # all close to 1 (range .8-1)

dry_dry_wet <- filter(sp.res.env.long, env.series.long == "dry_dry_wet")
hist(dry_dry_wet$gr.an) # all close to 1 (range .8-1)

dry_wet_wet <- filter(sp.res.env.long, env.series.long == "dry_wet_wet")
hist(dry_wet_wet$gr.an) # all high and trimodal (range 6.5-8)

dry_wet_dry <- filter(sp.res.env.long, env.series.long == "dry_wet_dry")
hist(dry_wet_dry$gr.an) # all high and trimodal (range 6.5-8)

# it seems like y-2 and y-1 matter for determining growth rate
# so let's plot that coloring by y-2 and y-1
# this was probably all we needed to do, not all the stuff above
# oh well...
ggplot(sp.res.env.long) +
  geom_histogram(aes(gr.an, fill=as.factor(env.series.long), aes=0.5))
